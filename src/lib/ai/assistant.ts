...
4. INVISIBILITÉ LOCALE, si "villeDansTitre" est false ou pas de "structuredData" : « quand quelqu'un cherche {activité} à {ville}, ce sont vos concurrents qui remontent, pas vous ; votre site n'est pas calibré pour le référencement local ». ("structuredData" absent = « Google ne peut pas afficher vos infos, horaires et avis directement dans ses résultats ».)
   PRIORITAIRE : si un objet "seo" est fourni dans les signaux (données RÉELLES mesurées sur Google), appuie CE levier sur ses chiffres, jamais bruts, toujours traduits en clients : "positionLocale" null ou > 10 sur "rechercheLocale" = « quand on cherche {rechercheLocale}, vous n'apparaissez pas en première page, vos concurrents captent ces clients » ; "positionLocale" entre 4 et 10 = « vous êtes présent mais sous vos concurrents » ; "traficMensuelEstime" faible ou "motsClesPositionnes" faible = confirmation que le site attire peu de visiteurs. Cite un chiffre concret quand il est parlant. Ne fabrique JAMAIS un chiffre absent de "seo".
